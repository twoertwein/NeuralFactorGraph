...
import utils
from message import FactorGraph, Messages


class DynamicCRF(nn.Module):
    def __init__(
        self, args, word_freq, langs, char_vocab_size, word_vocab_size, uniqueTags
    ):
        ...
        if self.model_type == "mono" or self.model_type == "baseline":
            self.hidden2tag = nn.Linear(
                2 * self.hidden_dim, self.uniqueTags.tagSetSize()
            )

        elif self.model_type == "specific" or self.model_type == "joint":
            self.hidden2tag_1 = nn.Linear(
                2 * self.hidden_dim, self.uniqueTags.tagSetSize()
            )
            self.hidden2tag_2 = nn.Linear(
                2 * self.hidden_dim, self.uniqueTags.tagSetSize()
            )
        ...
        if self.model_type == "joint":
            self.joint_tf1 = nn.Linear(
                2 * self.hidden_dim, self.uniqueTags.tagSetSize(), bias=False
            )
            self.joint_tf2 = nn.Linear(
                self.uniqueTags.tagSetSize(), len(self.langs), bias=False
            )

        self.pairs = list(itertools.combinations(range(self.uniqueTags.size()), 2))
        self.lstm_weights = nn.ParameterList(
            [nn.Parameter(torch.randn(t.size())) for t in self.uniqueTags]
        )
        ...
        if not self.no_pairwise:
            self.pairwise_weights = nn.ParameterList(
                [
                    nn.Parameter(
                        torch.zeros(
                            self.uniqueTags.getTagbyIdx(i).size(),
                            self.uniqueTags.getTagbyIdx(j).size(),
                        )
                    )
        ...
        if not self.no_transitions:
            self.transition_weights = nn.ParameterList(
                [nn.Parameter(torch.zeros(t.size(), t.size())) for t in self.uniqueTags]
            )

        if self.model_type == "specific":
            self.lang_pairwise_weights = nn.ParameterList(
                [
                    nn.Parameter(
                        torch.zeros(
                            len(self.langs),
            ...
            self.lang_transition_weights = nn.ParameterList(
                [
                    nn.Parameter(torch.zeros(len(self.langs), t.size(), t.size()))
    ...
    def get_lstm_features(self, words, word_idxs=None, lang=None, test=False):
        ...
        if self.model_type == "joint" and not test:
            tag_scores = tag_scores + tf2[:, self.langs.index(lang)].repeat(
                1, self.uniqueTags.tagSetSize()
            )
        elif self.model_type == "joint" and test:
            tag_scores = tag_scores + pred_lang_scores.repeat(
                1, self.uniqueTags.tagSetSize()
            )
    ...
    # @profile
    def belief_propogation_log(
        self, gold_tags, lang, sentLen, batch_lstm_feats, test=False
    ):
            ...
            if self.model_type == "specific":
                for i in range(len(self.pairs)):
                    pairwise_weights_np[i] = utils.logSumExp(
                        pairwise_weights_np[i],
                        self.lang_pairwise_weights[i][langIdx].cpu().data.numpy(),
                    )
        ...
        if not self.no_transitions:
            ...
            for tag in self.uniqueTags:
                transition_weights_np[tag.idx] = (
                    self.transition_weights[tag.idx].cpu().data.numpy()
                )
            ...
            if self.model_type == "specific":
                for tag in self.uniqueTags:
                    transition_weights_np[tag.idx] = utils.logSumExp(
                        transition_weights_np[tag.idx],
                        self.lang_transition_weights[tag.idx][langIdx]
                        .cpu()
                        .data.numpy(),
                    )
                ...
                for t in range(sentLen):
                    ...
                    for factor in factor_list:
                        if factor.kind == "pair":
                            ...
                            message = (
                                factor_sum - messages.getMessage(factor, var).value
                            )
                            ...
                            maxVal = np.maximum(
                                maxVal, np.amax(np.abs(curVal - pairwise_pot), 1)
                            )
                    ...
                    if not self.no_transitions:
                        ...
                        if t != sentLen - 1:
                            ...
                            message = (
                                factor_sum
                                - messages.getMessage(trans_factor, var).value
                            )
                            ...
                            transition_pot = utils.logDot(
                                messages.getMessage(var, trans_factor).value,
                                cur_tag_weights,
                                redAxis=0,
                            )
                            ...
                            maxVal = np.maximum(
                                maxVal, np.amax(np.abs(curVal - transition_pot), 1)
                            )
        ...
        for tag in self.uniqueTags:
            for t in range(sentLen):
                ...
                for factor in factor_list:
                    factorMsg = Variable(
                        torch.FloatTensor(messages.getMessage(factor, var).value)
                    )
                    if self.gpu:
        ...
        for factor in graph.iterFactors():
            ...
            if factor.kind == "trans":
                ...
                if self.model_type == "specific":
                    factor.belief = utils.logSumExpTensors(
                        factor.belief, self.lang_pairwise_weights[pairwise_idx][langIdx]
                    )
            else:
                continue

            factor.belief = factor.belief.view(1, factor.belief.size(0), -1).expand(
                batch_size, -1, -1
            )
            ...
            factor.belief = (
                Variable(msg1.view(batch_size, -1, 1).expand(-1, -1, var2.tag.size()))
                + factor.belief
            )
            factor.belief = (
                Variable(msg2.view(batch_size, 1, -1).expand(-1, var1.tag.size(), -1))
                + factor.belief
            )
    ...
    def calc_likelihood(self, graph, gold_tags):
        ...
        for tags, next_tags in zip(gold_tags, gold_tags[1:]):
            ...
            for i, tag in enumerate(cur_tags):
                ...
                cur_label_idx = self.uniqueTags.getTagbyIdx(tag_idx).label2idx[
                    tags_dict[tag]
                ]
                ...
                if tag in next_tags_dict:
                    ...
                    next_label_idx = self.uniqueTags.getTagbyIdx(tag_idx).label2idx[
                        next_tags_dict[tag]
                    ]
                    num += graph.getFactorByVars(var1, var2).belief[cur_label_idx][
                        next_label_idx
                    ]
                ...
                for coocc_tag in cur_tags[i:]:
                    if tag != coocc_tag:
                        ...
                        coocc_label_idx = self.uniqueTags.getTagbyIdx(
                            coocc_tag_idx
                        ).label2idx[tags_dict[coocc_tag]]
        ...
        for i, tag in enumerate(last_tags):
            ...
            for coocc_tag in last_tags[i:]:
                if tag != coocc_tag:
                    ...
                    coocc_label_idx = self.uniqueTags.getTagbyIdx(
                        coocc_tag_idx
                    ).label2idx[last_tags_dict[coocc_tag]]
                    factor = graph.getFactorByVars(var1, var2)
                    if factor.var1.tag.name == tag:
                        num += factor.belief[cur_label_idx][coocc_label_idx]
                    else:
    ...
    def get_scores(self, graph, gold_tags, lstm_feats, batchIdx):
        ...
        for tag in self.uniqueTags:
            ...
            for t in range(graph.T):
                cur_lstm_feats = lstm_feats[t]
                cur_tag_lstm_feats = cur_lstm_feats[
                    self.tag_offsets[tag.name] : self.tag_offsets[tag.name] + tag.size()
                ]
                cur_tag_lstm_weights = self.lstm_weights[tag.idx]
            ...
            for i, tag in enumerate(cur_tags):
                    ...
                    for coocc_tag in cur_tags[i:]:
                        if tag != coocc_tag:
                            ...
                            if var2.tag.idx < var1.tag.idx:
                                pairwise_idx = self.pairs.index(
                                    (var2.tag.idx, var1.tag.idx)
                                )
                                gold_belief = (
                                    coocc_label_idx * var1.tag.size() + cur_label_idx
                                )
                            else:
                                pairwise_idx = self.pairs.index(
                                    (var1.tag.idx, var2.tag.idx)
                                )
                                gold_belief = (
                                    cur_label_idx * var2.tag.size() + coocc_label_idx
                                )

                            target_belief = graph.getFactorByVars(var1, var2).belief[
                                batchIdx
                            ]
                ...
                if not self.no_transitions:
                    ...
                    if t != len(gold_tags) - 1:
                        ...
                        if tag in next_tags_dict:
                            var2 = graph.getVarByTimestepnTag(t + 1, tag_idx)
                            next_label_idx = self.uniqueTags.getTagbyIdx(
                                tag_idx
                            ).label2idx[next_tags_dict[tag]]
                            gold_belief = (
                                cur_label_idx * var1.tag.size() + next_label_idx
                            )
                            target_belief = graph.getFactorByVars(var1, var2).belief[
                                batchIdx
                            ]
            ...
            for factor in all_factors:
                ...
                for k in beliefs.keys():
                    if k in scores and k in beliefs:
                        ...
                        score = Variable(
                            torch.LongTensor(scores[k]), requires_grad=False
                        )
                        if self.gpu:
                            score = score.cuda()
    ...
    def forward(self, sents, gold_tags, word_idxs=None, langs=None, test=False):
        ...
        graph, maxVal = self.belief_propogation_log(
            gold_tags, lang, len(lstm_feat_sents[0]), lstm_feat_sents, test=test
        )
